...
def analyze_bathy_collect_short(xyz, epoch, data, cam, bathy, long_ok_flag=False):
    ...
    if not long_ok_flag:
        min_bands_for_v2 = 5
        not_short_record_length = min_bands_for_v2 / np.mean(np.diff(bathy['params']['fB'])) / (epoch[1] - epoch[0])
        if len(data) >= not_short_record_length:
            print(f'Warning - data record length {len(data)}s is longer than recommended.')
    ...
    for xind in range(len(bathy['xm'])):
        ...
        if bathy['params']['debug']['DOSHOWPROGRESS']:
            for yind in range(len(bathy['ym'])):
                fDep[yind], camUsed[yind] = csm_invert_k_alpha_short(f, G, xyz[:, :2], cam, bathy['xm'][xind], bathy['ym'][yind], bathy)
        else:
            for yind in range(len(bathy['ym'])):
                fDep[yind], camUsed[yind] = csm_invert_k_alpha_short(f, G, xyz[:, :2], cam, bathy['xm'][xind], bathy['ym'][yind], bathy)
