...
def csm_invert_k_alpha(f, G, xy, cam, xm, ym, bathy):
    ...
    for i in range(len(fs)):
        ...
        dxmi = xy[:, 0] - xm
        dymi = xy[:, 1] - ym
        r = np.sqrt((dxmi / bathy['params']['Lx']) ** 2 + (dymi / bathy['params']['Ly']) ** 2)
        ...
        try:
            ...
            popt, pcov = curve_fit(predict_csm, np.column_stack((xy, w)), np.concatenate([np.real(v), np.imag(v)]),
                                   p0=kAlphaInit, maxfev=2000)
            kAlpha = popt

            if kAlpha[0] < kmin or kAlpha[0] > kmax or kAlpha[1] > np.pi / 2 or kAlpha[1] < -np.pi / 2:
            ...
            skill = 1 - np.linalg.norm(vPred - v) ** 2 / np.linalg.norm(v - np.mean(v)) ** 2
        ...
        if not np.isnan(kAlpha[0]):
            ...
            fDependent['hTempErr'][i] = np.sqrt((np.interp(kAlpha[0], kii[1:], dhiidkii) ** 2) * (ex[0] ** 2))
        else:
