...
def csm_invert_k_alpha(f, G, xy, cam, xm, ym, bathy):
    fs, kAlpha0, subvs, subXY, camUsed, lam1Norms, centerInds = prepare_tiles(f, G, xy, cam, xm, ym, bathy)
    ...
    for i in range(len(fs)):
        ...
        result = least_squares(residuals, kAlphaPhi, args=(subXY[i], subvs[i]), method='lm')
